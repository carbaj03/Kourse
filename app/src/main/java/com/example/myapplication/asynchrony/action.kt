...
inline fun <reified S : State, E> actionSlice(
    ...
    crossinline f: suspend context(Raise<E>, Dispatcher) (S) -> Unit,
...
fun <S : State, E> action(
    ...
    @BuilderInference f: suspend context(Raise<E>, Dispatcher) (S) -> Unit
): AsyncAction<S> =
    createAction { s, d ->
        key?.let { jobs[key]?.invoke() }
        eitherIo(
...
fun <S : State, E> action2(
    key: String? = null,
    onError: suspend context(Dispatcher) (E) -> Unit = { _ -> },
    f: suspend context(Raise<E>, Dispatcher, Copy<S>) () -> Unit
): AsyncAction1<S> =
    createAction1 { s, d ->
        key?.let { jobs[key]?.invoke() }
        eitherIo(
...
fun <S : State, E> action1(
    key: String? = null,
    onError: suspend context(Dispatcher) (E) -> Unit = { _ -> },
    f: suspend context(Raise<E>, Dispatcher, Copy<S>) (S, () -> S) -> Unit
): AsyncAction1<S> =
    createAction1 { s, d ->
        key?.let { jobs[key]?.invoke() }
        eitherIo(
...
fun <S : State, E> actionMain(
    f: suspend context(Raise<E>, Dispatcher) (S) -> Unit
): AsyncAction<S> =
    createAction { s, d ->
...
@OptIn(ExperimentalTypeInference::class)
fun <S : State, E : Error> dispatchAction(
    key: String? = null,
...
context(WithScope, Store<S>)
fun <S : State> dispatchAction(
    key: String? = null,
    f: suspend context(Raise<Error>, Dispatcher) (S) -> Unit
) {
    action(key = key, f = f).dispatch()
}

fun <S : State> Store<S>.dispatch(f: S.() -> S) {
